...
async def websocket_endpoint(websocket: WebSocket):
    ...
    try:
            ...
            try:
                ...
                try:
                    ...
                    if base_msg.type == "draw_stroke":
                        ...
                        log.warning(f"Unknown message type '{base_msg.type}' received after base parsing.")
                        await websocket.send_json({"type": "error", "message": f"Unknown message type: {base_msg.type}"})
                        continue # Skip processing this message

                except ValidationError as e:
                    # Pydantic validation failed! Log the detailed error.
                    log.error(f"Validation failed for data: {raw_data}\nError: {e.json(indent=2)}") # Log detailed validation error
                ...
                if session is None:
                    try:
                        ...
                        log.info(f"Created AssessmentSession for task '{task_id_to_use}' for {websocket.client}. Total sessions: {len(active_sessions)}")
                    except ValueError as e:
                        log.error(f"Failed to create session for task '{task_id_to_use}': {e}")
                        await websocket.send_json({"type": "error", "message": f"Failed to load task rules for '{task_id_to_use}'."})
                        break # Close connection if session fails to initialize (e.g., bad rules file)
                    except Exception as e:
                        log.error(f"Unexpected error creating session for task '{task_id_to_use}': {e}", exc_info=True)
                        await websocket.send_json({"type": "error", "message": "Internal server error during session creation."})
                ...
                if session: # Should always be true after the block above, but check anyway
                    try:
                        action_to_send = session.process_event(validated_data) # Pass validated Pydantic model
                        ...
                        if session.assessment_complete:
                            log.info(f"Assessment complete flag set for session with {websocket.client}.")
                ...
                await websocket.send_json({"type": "error", "message": "Invalid data format. Please send JSON."})
                # Continue waiting for the next message
            except Exception as e:
                # Catch other potential receive errors or unexpected issues in the loop
                log.error(f"Error during WebSocket receive/process loop for {websocket.client}: {e}", exc_info=True)
        ...
        if websocket in active_sessions:
            ...
            log.warning("WebSocket not found in active sessions during cleanup (might have failed before session creation).")
        ...
        try:
            ...
            if websocket.client_state == websocket.client_state.CONNECTED or websocket.client_state == websocket.client_state.CONNECTING:
                ...
                log.info(f"WebSocket connection explicitly closed for {websocket.client} during cleanup.")
        except RuntimeError as e:
             # Can happen if the connection is already closing or closed abruptly
             log.warning(f"RuntimeError closing websocket during cleanup (likely already closed): {e}")
        except Exception as e:
